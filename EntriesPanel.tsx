...
const EntriesPanel: FC<EntriesPanelProps> = ({ category }) => {
  ...
  const { data: draws, isLoading, error } = useQuery<LotteryDraw[]>({
    ...
    queryFn: async () => {
      const response = await apiRequest("GET", `/api/draws/${category}`);
      return response.json() as LotteryDraw[];
    },
  });
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      ball1: 0, // Default to 0 to avoid undefined
      ball2: 0,
      ball3: 0,
      ball4: 0,
      ball5: 0,
  ...
  const createDrawMutation = useMutation<LotteryDraw, Error, InsertLotteryDraw>({
  ...
  const deleteDrawMutation = useMutation<void, Error, number>({
